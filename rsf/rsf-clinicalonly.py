...
feature_cols = [c for c in covariates if c not in ['OS_STATUS','OS_MONTHS']]
X_train = train[feature_cols]

num_of_cov = len(feature_cols)
...
rsf = RandomSurvivalForest(
    ...
    max_depth=20,
    min_samples_leaf=50,
    max_features=0.3,  # 0.5 * 13062 = 6531
    random_state=42,
    n_jobs=-1
)
...
model_file = os.path.join(output_dir, f"{current_date}_rsf_model-{len(rsf.estimators_)}-trees-maxdepth-{rsf.max_depth}-{len(feature_cols)}-features.pkl")
...
with open(os.path.join(output_dir, f"{current_date}_rsf_model_spec-{len(rsf.estimators_)}-trees-maxdepth-{rsf.max_depth}-{len(feature_cols)}-features.md"), "w") as f:
    f.write(f"# RSF Model Specification:\n")
    f.write(f"Model file: {model_file}\n")
    f.write(f"Number of covariates: {len(feature_cols)}\n")
