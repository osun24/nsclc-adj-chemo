...
GSE68465.data = GSE68465.data[, c(1, 2, 5, 8, 9, 12, 13, 14, 16)]

colnames(GSE68465.data) = c("Age", "Adjuvant Chemo", "Histology", "PFS_MONTHS", "OS_MONTHS", "Race", "Sex", "Smoked?", "OS_STATUS")

unique(GSE68465.data$`Smoked?`)
...
GSE68465.data$`Smoked?` <- ifelse(GSE68465.data$`Smoked?` %in% c("Smoked in the past", "Currently smoking"), 1,
                                  ifelse(GSE68465.data$`Smoked?` == "Never smoked", 0, NA))
...
GSE68465.expr = GSE68465.merged[, -c(1:9)]
GSE68465data.expr = GSE68465.merged[, 1:9]
```
