...
for (GSE in GSE_numbers) {
 ...
 if (GSE == "GSE68465") {
  ...
  if (GSE == "GSE14814") {
    ...
    # Keep the 6 relevant columns in order, then standardize names
...
for (GSE in GSE_numbers) {
  ...
  # Recode Adjuvant Chemo → numeric 1/0
...
cat("Total patients:", nrow(affy.data.filtered), "\n")

# ACT vs OBS
cat("Adjuvant Chemotherapy:\n")
print(table(affy.data.filtered$`Adjuvant Chemo`))

# Sex (from IS_MALE)
cat("Sex:\n")
cat("Male:", sum(affy.data.filtered$IS_MALE == 1, na.rm = TRUE), "\n")
cat("Female:", sum(affy.data.filtered$IS_MALE == 0, na.rm = TRUE), "\n")

# Age summary
cat("Age:\n")
cat("Mean age:", round(mean(affy.data.filtered$Age, na.rm = TRUE), 1), "\n")
cat("Median age:", round(median(affy.data.filtered$Age, na.rm = TRUE), 1), "\n")
cat("Range:", paste(range(affy.data.filtered$Age, na.rm = T), collapse = " - "), "\n")

# Stage breakdown
cat("Stage distribution:\n")
print(table(affy.data.filtered$Stage))

# Histology breakdown
cat("Histology:\n")
print(table(affy.data.filtered$Histology))

# Smoking status
cat("Smoking History:\n")
print(table(affy.data.filtered$`Smoked?`))

# Race
cat("Race:\n")
print(table(affy.data.filtered$Race))
```
