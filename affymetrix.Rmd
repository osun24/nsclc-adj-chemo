...
# Add lines for 2-fold above and below median
...
legend("topright", legend = c("Median", "2-fold thresholds"),
       col = c("blue", "red"), lty = c(2, 3), bty = "n", lwd = 2)
...
# Match outlier samples to their batches
outlier_batches <- affy.expr.batch[upper_outliers]

# View which batches the outliers came from
table(outlier_batches)
...
# Subset PCA coordinates for these samples
pca_after_df <- as.data.frame(affy.pca.after$x)
pca_after_df <- pca_after_df[kept_samples, ]
pca_after_df$SampleID <- rownames(pca_after_df)

# Subset the batch info to match
pca_after_df$Batch <- affy.expr.batch[kept_samples]

# Recalculate percent variance explained (optional if unchanged)
explained_var_after <- affy.pca.after$sdev^2 / sum(affy.pca.after$sdev^2)
percent_var_after <- round(100 * explained_var_after[1:2], 1)
...
compute_percent_a <- function(call_matrix) {
  if (!is.matrix(call_matrix)) return(NULL)

  percent_a <- colMeans(call_matrix == "A") * 100
...
gpl570_a_list <- lapply(gpl570_results$call_matrices, compute_percent_a)
gpl96_a_list  <- lapply(gpl96_results$call_matrices,  compute_percent_a)

library(ggplot2)

gpl570_a_df <- do.call(rbind, Map(function(df, name) {
  if (!is.null(df)) {
    df$Dataset <- name
    df$Platform <- "GPL570"
    return(df)
  }
}, gpl570_a_list, names(gpl570_a_list)))

gpl96_a_df <- do.call(rbind, Map(function(df, name) {
  if (!is.null(df)) {
    df$Dataset <- name
    df$Platform <- "GPL96"
    return(df)
  }
}, gpl96_a_list, names(gpl96_a_list)))

combined_a_df <- rbind(gpl570_a_df, gpl96_a_df)

ggplot(combined_a_df, aes(x = Percent_A, fill = Platform)) +
  ...
  labs(
    title = "Absent Call Rate per Array",
    x = "% Absent Calls",
    y = "Number of Arrays",
    fill = "Platform"
  ) +
...
gpl570_gses <- c("GSE157010", "GSE29013", "GSE31210", "GSE31908", "GSE37745", "GSE50081")
base_dir <- "~/GitHub/nsclc-adj-chemo"

# Step 1: Collect all CEL file paths
all_cel_files <- c()

for (gse in gpl570_gses) {
  cel_path <- file.path(base_dir, gse, "CEL")
  cel_files <- list.files(cel_path, pattern = ".CEL.gz", full.names = TRUE)
  
  # GSE31908 has 2 platforms, use only the first 17 (GPL570)
  if (gse == "GSE31908") {
    cel_files <- cel_files[1:17]
  }
  
  all_cel_files <- c(all_cel_files, cel_files)
}

# Read all CEL files into a single AffyBatch object
gpl570_affy <- ReadAffy(filenames = all_cel_files)

# 1a. MAS5 expression for intensity ranking
eset_full  <- mas5(gpl570_affy)          
exprs_mat  <- exprs(eset_full)
mean_int   <- rowMeans(exprs_mat)

# 1b. Define top‑80% cutoff
cutoff     <- quantile(mean_int, .20)
keep_probe <- mean_int > cutoff

# 2. Get all calls 
calls_full <- mas5calls(gpl570_affy)     

# 3. Subset to just the top‑80% probes
calls_top80 <- calls_full[keep_probe, ]  

# sanity check
dim(exprs(calls_top80))

# 1. Extract the PMA call matrix (probes × samples)
calls_mat <- exprs(calls_top80)

# 2. Compute % Absent per sample
pctA <- colSums(calls_mat == "A") / nrow(calls_mat) * 100

# 3. Build a data frame for plotting
df_pctA <- data.frame(
  Sample = factor(colnames(calls_mat), levels = colnames(calls_mat)), 
  pctA   = pctA
)

# 4. Plot with ggplot2
ggplot(df_pctA, aes(x = pctA)) +
  geom_histogram(position = "identity", bins = 30, alpha = 0.7, color = "black") +
  scale_fill_manual(values = c("GPL570" = "gray40")) +
  geom_vline(xintercept = 10, color = "red", linetype = "dashed") +
  labs(
    title = "Absent Call Rate per Array",
    x = "% Absent Calls",
...
ggplot(leverage_df, aes(x = Leverage)) +
  ...
  geom_vline(xintercept = q99, color = "red", linetype = "dotted", size = 1) +
  theme_minimal() +
  labs(
...
top10 = sort(leverage, decreasing = T)[1:10]

quantile_9973 <- quantile(leverage, probs = 0.995)
print(quantile_9973)
