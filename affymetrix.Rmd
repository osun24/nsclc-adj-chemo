...
mnn.corrected <- fastMNN(affy.expr.all.t)

# Extract the corrected expression matrix
corrected.matrix <- reducedDim(mnn.corrected, "corrected")
rownames(corrected.matrix) <- rownames(affy.expr.all)

affy.mnn.corrected <- do.call(mnnCorrect, affy.expr.all.t)
corrected.expr <- assay(affy.mnn.corrected)

affy.pca.after <- prcomp(t(corrected.expr), scale=TRUE)

fviz_pca_ind(affy.pca.after,
             geom = "point",
             col.ind = affy.expr.batch,
             addEllipses = TRUE,
             legend.title = "After BatMan (fastMNN) Correction")
```
