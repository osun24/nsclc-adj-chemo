...
CACHE_FILE = os.path.join(BASE_DIR, 'feature_descriptions_cache.json')
PROGRESS_FILE = os.path.join(BASE_DIR, 'background_progress.json')
...
# Load RSF model
RSF_MODEL_PATH = os.path.join(BASE_DIR, 'rsf_final.joblib')
FEATURES_USED_PATH = os.path.join(BASE_DIR, 'features_used.txt')
...
if hasattr(rsf, "feature_names_in_"):
    model_features = list(rsf.feature_names_in_)
else:
    if not os.path.exists(FEATURES_USED_PATH):
        raise ValueError("Model missing feature_names_in_ and features_used.txt not found.")
    with open(FEATURES_USED_PATH, "r") as f:
        model_features = [line.strip() for line in f.readlines() if line.strip()]

covariates = [f for f in model_features if f in train.columns]
